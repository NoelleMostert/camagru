...
addtogalleryButton.addEventListener('click', function(e){
    ...
    for (var x = 0; x < items.length; x++)
    {
        ...
        ajax.open("POST", "./upload.php?type=gallery", true);
        ajax.send(formdata);
...
function takePicture() {
    ...
    if(width && height){
        ...
        // capture_div.setAttribute("onclick", OnClickInfo());

        //trying something new//
        // Creating a counter that will allow for a unique ID to be generated for each image
       ...
       // counter.innerHTML = 0;
        ...
        if (counter.innerHTML == '') {
            ...
            capture_div.setAttribute("id", "i" + $currentCount);
            counter.innerHTML++;
        }
        else {
            $currentCount = counter.innerHTML;
        ...
        // document.getElementById("i"+$currentCount).addEventListener("click", function() {
        //     removeElement("i"+$currentCount)
        // })

        document.body.addEventListener("click", function(res) {
            if (res.path[1].className === "upload_item") {
                removeElement(res.path[1].id);
            }
        })
